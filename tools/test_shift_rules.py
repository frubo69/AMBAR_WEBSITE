...
async def main():
    ...
    ЛЮДИ = [
        ...
        {"_id": "Файзуло", "work": [{"from": "2026-01-01", "to": DAY}]},       # уезжает СЕГОДНЯ
        {"_id": "Завтра", "work": [{"from": "2026-01-01", "to": "2026-09-23"}]},  # уезжает завтра
    ...
    eq("улетевшего и того, кто уезжает сегодня, не предлагаем",
       await opr._crew_names("jvc", DAY), ["Худоба", "Али", "Завтра", "Вернулся"])
    eq("кто уезжает завтра — ещё в списке", "Завтра" in await opr._crew_names("jvc", DAY), True)
    ...
    eq("уже отмеченного не прячем, даже если он улетел",
       ...
       ["Худоба", "Али", "Завтра", "Улетел", "Вернулся"])
    ...
    st, b = await открыть({"Али": False, "Завтра": False})
    ...
    st, b = await открыть({"Али": True, "Завтра": False, "Файзуло": True, "Улетел": True})
    eq("хотя бы один вышел — открыта", (st, b.get("ok")), (200, True))
    eq("в бригаде только те, кто в Дубае", SHIFT.get("drivers"), {"Али": True, "Завтра": False})
    eq("уезжающего сегодня не отметить даже прямым запросом", "Файзуло" in (SHIFT.get("drivers") or {}), False)
    eq("день проставлен им же", SAVED, {"Али": {"working": True}, "Завтра": {"working": False}})
    eq("сообщение — им же", sorted(TOLD), ["Али", "Завтра"])
    ...
    eq("остальные на месте", OPENS_OP["jvc"]["drivers"], {"Али": True, "Завтра": False, "Худоба": True})
    SAVED.clear(); TOLD.clear()
    st, b = await бригада({"Худоба": True})
    ...
    st, b = await бригада({"Вернулся": True, "Завтра": True})
    eq("двоих разом", (st, sorted(b.get("changed"))), (200, ["Вернулся", "Завтра"]))
    st, b = await бригада({"Улетел": True})
    eq("улетевшего добавить нельзя — правок нет", (st, b.get("changed")), (200, []))
    st, b = await бригада({"Али": False, "Вернулся": False, "Завтра": False})
    eq("совсем без водителей оставить нельзя", (st, b.get("error")), (409, "last_driver"))
    eq("бригада не тронута", OPENS_OP["jvc"]["drivers"],
       {"Али": True, "Завтра": True, "Худоба": False, "Вернулся": True})
