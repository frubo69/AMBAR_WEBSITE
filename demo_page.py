...
прошлогодний экран, хуже, чем никакого. Подменяются ровно две вещи:

  • api.js                    → driver/demo.js (сервер, живущий во вкладке);
  • boot()                    → demoBoot() (чинит камеру и сканер, зовёт boot).

Скрипт телеграма остаётся на месте (владелец, 18 сен 2026: «сделай так же на
базе телеграма»): открытое кнопкой web_app демо получает настоящий телеграм —
полный экран, свою шапку, отклик кнопок, подтверждение выхода, — то есть ведёт
себя ровно как приложение, а не как страница в браузере. Заглушку телеграма
demo.js ставит только там, где телеграма нет вовсе.
...
def build(src: str, ver: str = "") -> str:
    """Две подмены. Если разметка изменилась так, что подменять нечего —
    честно падаем: молча отданное демо без своего сервера ушло бы в боевой
    и показало бы человеку экран «нет доступа»."""
    tag = f'<script src="/driver/{DEMO_JS}{ver}"></script>'
    for need, rep in ((API_TAG, tag), (BOOT, "\ndemoBoot();")):
        if src.count(need) != 1:
