...
async def tasks_for_driver(me: str, district: str) -> dict:
    ...
    # Одна точка — одна задача: та, что из самой свежей поставки.
    #
    # Заявку пересобирают от того, чего не хватает сейчас, поэтому новая
    # поставка по той же точке отменяет старую собой: ехать за августовским
    # заказом в сентябре некуда, магазин его давно не держит. Пока этого
    # правила не было, поставки просто копились — водитель видел девять
    # карточек вместо пяти, половина из них мёртвые, и отличить живую от
    # остатка было нечем.
    #
    # Исключение одно: если в старую задачу уже приняли бутылки, она остаётся.
    # Те бутылки записаны в тот документ, и бросить его — значит потерять
    # приёмку на полпути. Закрыть её должен человек, а не правило.
    mine, free, other = [], [], []
    свежая: dict[str, str] = {}                 # точка → id самой новой поставки
    for sup in await db.supplies_with_open_tasks(limit=8):
        ...
        for oid, task in (sup.get("tasks") or {}).items():
            ...
            # Поставки приходят от новых к старым: первая встреченная точка и
            # есть текущая.
            текущая = свежая.setdefault(oid, sid) == sid
            if not текущая and not v["got"]:
                continue
            v["home"] = (oid == district)
            v["stale"] = not текущая            # незакончено с прошлой поставки
            if v["mine"]:
                mine.append(v)
            elif v["driver"]:
                other.append(v)
