...
(function () {
    ...
    switch (m.t) {
      ...
      case 'camallow':
        ...
        if (!m.own) {
          ...
          // Оператор включил камеру за меня — поднимаем её сами. Не дали
          // камеру — останемся голосом, экран узнает от _camOn.
          else if (m.force && this.call && !this.camLive()) {
            this._camOn().catch(function () {});
          }
        }
  ...
  // force — включить камеру собеседнику самому: его телефон поднимет её без
  // нажатия. Выключение и так выключает: запрет гасит камеру на той стороне.
  AmbarCall.prototype.allowCam = function (on, force) {
    this._send({t: 'camallow', on: !!on, force: !!force});
  };
  ...
  // Тот же алфавит нужен экрану: барабан прокручивает мимо не что попало, а
  // только тех кандидатов, которые действительно могли выпасть.
  AmbarCall.EMOJI = СМАЙЛЫ;
