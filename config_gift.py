...
Подарок добавляет СЕРВЕР. Клиент присылает только состав корзины: если бы
подарок приходил с фронтенда, любой POST мог бы положить в заказ бесплатную
бутылку без всякой тысячи.

Только заказы из приложения. Акция существует ради перехода в приложение — если
вино достаётся и по телефону, переходить незачем.
"""
