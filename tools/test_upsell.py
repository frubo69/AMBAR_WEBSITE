"""Чай водителю за допродажу (владелец, 17 сен 2026): 5% от цены позиций,
добавленных к заказу после того, как водитель его взял. На заказе копится
upsell, в день водителя ложится расход «Чай за допродажи» (остаётся у него),
...
async def main():
    ...
    eq("чай: 5% от 400 = 20 AED за 4 позиции", (r["aed"], r["bonus"], r["n"]), (400.0, 20, 4))
    ...
    eq("в дне водителя расход «Чай за допродажи» 20, согласован, по заказу", (len(ex), ex[0]["amount"], ex[0]["status"], ex[0]["order"], ex[0]["auto"]), (1, 20, "approved", "O1", True))
    ...
    dd = await db.get_driver_day("2026-09-17", "Худоба")
    eq("в дне две строки чая", sum(x["amount"] for x in dd["extras"] if x.get("kind") == "upsell"), 25)
    eq("правка без добавлений — чая нет", await op._upsell_credit("O1", o, new, new, "Худоба", "Оператор", "x"), None)
    eq("убавили — чая нет", await op._upsell_credit("O1", o, new, old, "Худоба", "Оператор", "x"), None)
    n = await db.driver_expense_pull_order("2026-09-17", "Худоба", "O1", "upsell")
    dd = await db.get_driver_day("2026-09-17", "Худоба")
    eq("отмена заказа снимает строки чая", (n, sum(x["amount"] for x in dd["extras"] if x.get("kind") == "upsell")), (1, 0))
