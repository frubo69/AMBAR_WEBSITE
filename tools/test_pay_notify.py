...
async def main():
    ...
    has("штраф отменён", pn.cancelled(it, "Старший"), "Штраф 200 AED отменён", "Превышение", "Отменил: Старший")
    ...
    has("отмена ушла", SENT[-1][1], "Штраф 150 AED отменён", "Отменил: Парвиз")
    st, r = await call(fin.handle_pay_item_del, {"id": iid, "as": "Парвиз"})
