...
EXTRA_KINDS = {
    ...
    # Охране платят бутылкой или наличными — одно из двух (владелец, 14 сен
    # 2026), поэтому «как платили» у неё не спрашиваем: nopay.
    "guard":   {"t": "Охрана", "nopay": True},
    "kfc":     {"t": "KFC · премия"},
    ...
    # Возвраты и долги — всегда наличными из рук в руки, «как платили» у них не
    # спрашиваем (владелец, 20 сен 2026: «отсюда убери способы оплаты»).
    "we_gave": {"t": "Мы вернули", "nopay": True},
    # «Нам должны» — денег никто не платил, это недостача: nopay.
    "owed_us": {"t": "Нам должны", "nopay": True},
    "we_got":  {"t": "Нам вернули",  "plus": True, "nopay": True},
    "we_owe":  {"t": "Мы должны",    "plus": True, "nopay": True},
    # Бонус водителя за допродажу: 5% от цены позиций, которые он добавил к
    # заказу уже после того, как взял его (владелец, 17 сен 2026). Пишется
    ...
    #
    # Чек тут по желанию, а не обязателен (владелец, 25 сен 2026: «чтобы сюда
    # можно было загружать чеки тоже»). Обязательным его делать нельзя: под
    # «что-то ещё» пишут и возвраты, и долги, где чека нет в природе, — а вот
    # у запчасти или у KFC он есть, и старшему его надо видеть. Поэтому
    # receipt_opt, а не receipt: плитка для снимка есть, но без него запись
    # всё равно уходит. MUST_RECEIPT в driver_routes смотрит только receipt.
    "other":   {"t": "Что-то ещё", "receipt_opt": True},
}
...
# Как платили (владелец, 18 сен 2026: «когда водитель заполняет расход,
# заставь его где-то выбрать, он заплатил наличными или безналичная оплата
# была»). Безналом — наличные водителя не тронуты: из «на руках», «к сдаче»
# и выручки дня такая трата не вычитается, а расходом дня остаётся. Записи
# без ответа (до 19 сен и из старого приложения) — наличные, как и были.
PAY_T = {"cash": "наличными", "card": "безналом"}


def is_card(e: dict) -> bool:
    return str((e or {}).get("pay") or "") == "card"
...
async def handle_extra_add(request):
    ...
    вид = EXTRA_KINDS[kid]
    if вид.get("auto"):
        return web.json_response({"error": "auto_only", "kind": kid}, status=400, headers=CORS_HEADERS)
