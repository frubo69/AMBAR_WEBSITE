...
# «Пока не выключу», а не восемь часов: восемь часов кончаются посреди смены и
# требуют включать заново каждый день. Телефон рабочий, а в отпуске трансляцию
# выключают руками — это одно движение и один раз.
HOW = ("📎 → «Геопозиция» → «Транслировать» → «Пока не выключу».")
...
async def tick(now: datetime = None) -> dict:
    ...
    for name in working:
        ...
        # Включают в отдельном боте геопозиции — туда и зовём, со ссылкой.
        try:
            import geo_watch
            how = await geo_watch.geo_how()
