...
async def main():
    ...
    # ручная правка владельца сильнее любого правила
    M.OVERRIDE = {"jvc": {fast: 7, slow: 4, dear: 1}}
    t2 = M.build(SR, rate)
    eq("правка владельца бьёт расчёт", t2[("jvc", fast)], 7)
    eq("правка владельца бьёт прочерк операторов", t2[("jvc", slow)], 4)
    eq("правка владельца бьёт запрет на дорогое", t2[("jvc", dear)], 1)
    eq("нетронутые клетки считаются как считались", t2[("bbay", cheap)], t[("bbay", cheap)])
    M.OVERRIDE = {}
