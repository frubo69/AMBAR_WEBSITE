...
async def handle_where(request):
    ...
    # просят тем же словом, а ключ точки остаётся по имени из .env. Устройства
    # (планшеты из бота устройств) — своей группой в конце списка, под своей
    # меткой: телефон старшего и его планшет — разные точки.
    P, D = geo_watch.SENIOR_PREFIX, geo_watch.DEVICE_PREFIX
    labels = {}                                  # ключ точки → (подпись, код, устройство?)
    for n in seniors:
        labels[P + n] = ("STAR", "STAR", False)
    for lb in staff.DEVICE_IDS:
        labels[D + lb] = (lb, staff.device_code(lb), True)
...
async def handle_pos(request):
    ...
    if not (-90 <= lat <= 90 and -180 <= lon <= 180):
        return web.json_response({"error": "bad_coords"}, status=400, headers=CORS_HEADERS)
    acc = body.get("acc")
    # Планшет точку из панели не шлёт: устройства транслируют в бот устройств,
    # а панель на планшете лежит на точке и только сбивала бы след телефона.
    # Старая страница может прислать device: tablet — принимаем и не пишем.
    if str(body.get("device") or "").strip().lower() == "tablet":
        return web.json_response({"ok": True, "ignored": "tablet"}, headers=CORS_HEADERS)
...
COVER_TEXT = (
    "🎮 *BLOCK STAR — классический тетрис*\n\n"
        ...
        for d in team:
            ...
            if w is not None:
                ...
                items.append({"id": "", "kind": "meal", "kind_t": "Питание", "who": d["name"],
                              "amount": meal, "at": "", "status": "approved",
                              "note": "на смене" if w else "дома", "photo": False})
            for e in r.get("extras") or []:
...
async def handle_checklist(request):
    ...
    try:
        ...
        wo = 0
    # Бутылки без кодов — тем же счётом, что экран «Внести / удалить товар»:
    # реестр против полки (где пересчёт есть) плюс принятое без сканирования
    # из незавершённого приёма (иначе 147 бутылок горели на экране, а чек-лист
    # писал «все в реестре» — владелец, 14 сен 2026). Район без пересчёта и
    # без такого приёма пункт закрыть не даёт: «всё в реестре» тут никто не
    # проверял.
    без_кодов, без_пересчёта = 0, []
    try:
        import qr_routes as QR
        import supply_routes as SUP
        по_реестру, без_пересчёта = await QR.unscanned_by_district()
        приём = {}
        for t in await SUP.noscan_tasks():
            приём[t["district"]] = приём.get(t["district"], 0) + int(t.get("left") or 0)
        # Незавершённый приём в базу количественного учёта ещё не вошёл (JVC:
        # по реестру 0, в приёме 147), поэтому складываем, а не выбираем.
        for oid in OFFICE_IDS:
            без_кодов += int(по_реестру.get(oid) or 0) + int(приём.get(oid) or 0)
        без_пересчёта = [o for o in без_пересчёта if not приём.get(o)]
    except Exception as e:                       # noqa: BLE001
    ...
    try:
        ...
        log.warning(f"[chk] сбор денег по районам: {e}")
        cr = {"done": 0, "need": 1 if cash else 0, "all_done": False, "net_total": cash}
    try:
        marks = await db.checklist_get(day)
    except Exception as e:
    ...
    rows = [
        ...
        _chk_row("unscanned", "QR код не внесён",
                 (f"{без_кодов} {_pl_bottles(без_кодов)} без QR кода" if без_кодов
                  else (f"пересчёта нет · {' '.join(OFFICE_CODES.get(o, o) for o in без_пересчёта)}"
                        if без_пересчёта else "все бутылки в реестре")),
                 без_кодов == 0 and not без_пересчёта, now, day, plan, go="qr", n=без_кодов),
    ...
    if sup["gap"]:
        rows.append(_chk_row("shortfall", "Докупить на базах",
