...
async def main():
    ...
    global первый_pid
    первый = первый_pid = next(iter(sr._catalog()))
...
async def следующий_день():
    """Вторая заморозка обязана считать заново, а не копировать вчерашнюю.

    Снимок дня ищется и по дню закупки: экран просит «на 25-е», документ лежит
    под 24-м. Для показа это правильно, для сборки — нет: freeze_order звал тот
    же расчёт и записал бы новую заявку копией вчерашней.
    """
    print("\n── следующий день: заявка собирается заново ──────────────────")
    ЗАВТРА = "2026-09-25"
    await подменить({первый_pid: 1})            # норма 10, на полке 1 → просим 9
    r = await sr.freeze_order(ЗАВТРА)
    eq("заморозили новый день", (r["ok"], r.get("already")), (True, None))
    d2 = await sr.order_rows(ЗАВТРА)
    eq("НОВАЯ ЗАЯВКА СЧИТАНА ЗАНОВО, а не скопирована", d2["total_qty"], 9)
