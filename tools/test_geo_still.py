...
async def watch():
    ...
    POS["Али"] = pos(30, 30, until="")
    out = await geo_watch.tick(NOW.astimezone(geo_watch.DUBAI_TZ))
    # Проход не знает, почему точек нет: выключил, вышел срок, уснул телефон.
    # Поэтому говорит «трансляции нет» и не штрафует (владелец, 24 сен 2026).
    eq("трансляции нет — так и говорим, один раз", (out.get("off"), [t for _, t in SENT]),
       (["Али"], ["📍 *Али*: трансляции нет\nОператор его не видит. "
                  "Мы не знаем, выключил он её или она кончилась сама, — поэтому без штрафа."]))
