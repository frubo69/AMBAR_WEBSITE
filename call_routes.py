...
RING_STEP = 5.0           # как часто перезванивает бот водителю
...
# видеть словом, а не догадываться по номеру: «Старший» и «fixxxik» — разные
...
#
# Одно сообщение — один «дзынь», прослушать легко.
#
# Во время звонка НИЧЕГО не стираем, и это главное. Удаление сообщения телеграм
# снимает вместе с ним и уведомление — у всего чата разом. Пока мы стирали
# предыдущую карточку каждую секунду, на заблокированном экране от звонка не
# оставалось ничего: уведомление приходило и тут же гасло. Пробовали и наоборот,
# сначала стереть, потом отправить, — тот же итог.
#
# Поэтому карточки копятся, пока идёт вызов, и стираются все разом, когда он
# кончился. В чате остаётся только след «пропущенный звонок». Раз в пять секунд
# за полторы минуты это восемнадцать карточек — стопка уведомлений на экране
# блокировки, которую и надо не пропустить.
def _drv_token() -> str:
...
async def _ring_driver(call: Call, name: str):
    ...
    try:
        ...
        # Звонок кончился — убираем всю стопку разом. Взял или не взял, карточки
        # с обратным отсчётом в переписке не нужны; не взял — вместо них
