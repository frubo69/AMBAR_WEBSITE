...
# Смену водитель открывает не позже 18:00 (владелец, 22 сен 2026: «это теперь
...
# ночью — тоже позже 18:00 этих суток.
SHIFT_LATE_HOUR = int(os.getenv("AMBAR_SHIFT_LATE_HOUR", "18"))
...
async def _late_alert(me: dict, now: datetime, day: str) -> None:
    """Смену открыли позже 18:00 — старшему (STAR) и операторам района; в STAR
...
async def handle_ping(request):
    ...
    # Что умеет камера этого телефона: у вспышки яркости нет ни у кого, а вот
    # ступени света в кадре одни телефоны дают, другие нет. Раз в день строкой
    # в журнал — иначе «почему у него линейка есть, а у меня нет» останется
    # догадкой. В базу не ложится и никого, кроме отладки, не касается.
    if q.get("cam"):
        log.info(f"[driver] {me['name']}: камера — {q['cam'][:200]}")
