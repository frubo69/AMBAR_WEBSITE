...
async def main():
    ...
    SNAP.clear()
    iso = (now - timedelta(hours=1)).replace(microsecond=0)
    st, body = await call("GET", f"?id=K7QW2XNP4A&lat=25&lon=55&timestamp={iso.strftime('%Y-%m-%dT%H:%M:%SZ')}")
    eq("ISO-время понимаем", (st, POS[-1]["at"].isoformat()), (200, iso.isoformat()))
