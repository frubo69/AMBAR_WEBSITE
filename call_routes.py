...
Кто кому звонит
---------------
У водителя есть СВОЙ оператор — тот, за кем закреплён его район
(config_staff.DISTRICT_STAFF). Но районный оператор в этой системе не аккаунт,
а имя: своего входа в телеграм у него нет, он выбирает себя в панели за общим
устройством. Значит и звонок адресуется не человеку, а тому, кто СЕЙЧАС
представлен этим именем в открытой панели. Никто не представлен — звонок уходит
старшему: он и так подменяет любого из районных, это его работа.
...
# видеть словом, а не догадываться по номеру: «Старший» и «Владелец» — разные
