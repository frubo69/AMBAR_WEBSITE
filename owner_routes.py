...
async def handle_checklist(request):
    ...
    # где пересчёт есть, реестр сравнивают с полкой; где пересчёта нет,
    # сравнивать не с чем, и считается принятое без сканирования (иначе 147
    # бутылок незавершённого приёма горели на экране, а чек-лист писал «все в
    # реестре» — владелец, 14 сен 2026). Район без пересчёта и без такого
    # приёма пункт закрыть не даёт: «всё в реестре» тут никто не проверял.
    ...
    try:
        ...
        for oid in OFFICE_IDS:
            без_кодов += int(по_реестру[oid]) if oid in по_реестру else int(приём.get(oid) or 0)
