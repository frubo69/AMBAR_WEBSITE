...
старший решает, любое решение — в историю с исходом. Два вида:
  ...
  • смену открыли позже 18:00 — «урезать питание» 80 → 40 за тот день
...
async def main():
    ...
    await dr._late_alert({"name": "Баха", "district": "bbay"}, at(18, 57), DAY)
    ...
    eq("водителю — сообщение: день, 40 вместо 80, за что", (TOLD[0][0], "40 AED вместо 80" in TOLD[0][1],
       "22 сентября" in TOLD[0][1], "Открытие смены позже 18:00 — смена открыта в 19:20" in _sp(TOLD[0][1])), ("Али", True, True, True))
    TOLD.clear()
    ...
    eq("поздняя смена по записи до нового поля (разбор подробностей)",
       fines_auto.full_text({"kind": "late_shift", "note": "открыл в 18:57, правило — до 18:00"}),
       "Открытие смены позже 18:00 — смена открыта в 18:57")
    ...
    eq("Баха: питание не урезали — «прощено», из зарплаты ничего",
       ...
       [("Открытие смены позже 18:00 — смена открыта в 18:57", True, True, 40, 0)])
