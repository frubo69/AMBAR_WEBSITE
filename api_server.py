...
async def on_startup(app):
    ...
    # Товар принят без сканирования: раз в час старшему, пока не досканируют.
    try:
        import supply_nag
...
async def _remember_driver_msg(token, chat_id, res):
    """Запомнить номер сообщения, отправленного водителю или оператору.
    ...
    Операторский бот сюда попадает наравне с водительским. Раньше не попадал, и
    штора у оператора убирала ровно те два сообщения, которые кто-то догадался
    записать руками, — остальное оставалось в чате и старело за грань двух
    суток, после которой удалить уже нельзя никогда. У оператора в чате те же
    заказы и те же адреса, и прятать их надо так же.

    Реестр общий: ключ в нём — чат, а не человек, и боту всё равно, кто на том
    конце. Кем удалять, свипер выясняет по чату отдельно.

    AMBAR STAR сюда не попадает: у него свой реестр и свой срок.
    """
    свои = {os.getenv("DRIVER_BOT_TOKEN", ""), os.getenv("OPERATOR_BOT_TOKEN", "")}
    свои.discard("")
    if token not in свои:
        return
