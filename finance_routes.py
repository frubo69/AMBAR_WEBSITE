...
async def handle_entry_add(request):
    ...
    # исключение (офисы, машины, реклама): их платят кнопкой «Оплатил» суммой
    # из плана, чека к ней нет (владелец, 12 сен: «нажал кнопку — оплатил,
    # расход засчитан»)
    rent = bool(ln) and _line_group(ln) in ("rent", "car", "ads")
