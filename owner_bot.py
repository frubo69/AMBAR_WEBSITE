...
async def on_any_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Кроме геопозиции. Живая трансляция «пока не выключу» — это одно сообщение
    старшего, которое телеграм правит каждые несколько секунд. Попав в реестр,
    оно стиралось чистильщиком ровно через восемь часов, и вместе с ним
    телеграм гасил трансляцию: старший её не выключал, а владельцы через
    четверть часа получали «не виден». 7 сентября: последняя точка 20:05:08,
    чистка 20:05:26. Точка старшего — не переписка, ей в реестре не место;
    тревожный вайп её тоже не тронет, и это правильно: в ней нет ничего, кроме
    того, где он стоит."""
    msg = update.effective_message
    if msg is not None and getattr(msg, "location", None):
        return
    await _track(msg)
