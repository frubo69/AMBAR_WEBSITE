...
def правила():
    ...
    eq("уехал и не вернулся — стаж замер на последнем рабочем дне (31 июля), не растёт",
       (s["months"], s["earned"], s["why"]), (18, 3000, "Уехал — стаж не идёт"))
    s = tenure.state(чел("Вернувшийся", [{"from": "2025-01-01", "to": "2026-06-30"},
                                         {"from": "2026-08-01", "to": ""}]),
    ...
    eq("водители, операторы и старший — да, офис — нет; порядок: старший, оператор, водители",
       [r["title"] for r in л], ["Старший", "Оператор", "Водитель"])
