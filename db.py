...
_DEFAULT_PREFS = {
    ...
    "qr.alien": True,
    # Наклейка, которой нет в реестре: решение за владельцем, и без этой вести
    # находка так и останется лежать в СТАРе незамеченной (25 сен 2026).
    "qr.found": True,
...
async def supply_take(sid: str, district: str, product_id: str, room, now, qty=1):
    """Принять один код по строке задачи: бутылку (1) или полкоробки пива (0.5).
...
async def supply_untake(sid: str, district: str, product_id: str, qty=1) -> bool:
    """Снять один код — водитель отменил последний скан (бутылку или полкоробки пива)."""
    db = _db_or_none()
    if db is None: return False
    r = await db.supplies.update_one(
...
# Код несёт количество: бутылка — 1, код пива — 0.5. QR клеится на коробку, а
# не на банку, и на каждой коробке два кода (владелец, 16–17 сен 2026: «мы
# вообще не считаем в банках… только ящики», «каждое сканирование пива это шаг
# +0.5»); сколько несёт код, решает stock_routes.code_qty. Поэтому все
...
async def intake_since(district: str, since, until=None) -> dict:
    ...
    # intake_extra — вторая половина коробки у кодов пива, принятых до 17 сен
    # 2026: приёмка тогда закрывалась одним кодом на коробку (qty 1). Код стал
...
async def audits_unfinished(before_day: str, limit: int = 20) -> dict:
    ...
    Брошенные (`dropped_at`) не показываем. Так бывает, когда район посчитали
    заново и довели до конца, а прежний заход остался висеть строкой «не
    завершена» и сбивал с толку (владелец, 25 сен 2026: «это неактуальная
    ревизия, которая ломает логику»). Сканы при этом остаются в базе: строку
    убираем с глаз, а не стираем — вернуть можно, сняв поле."""
    db = _db_or_none()
    if db is None: return {}
