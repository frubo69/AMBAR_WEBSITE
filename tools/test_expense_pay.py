...
или безналичная оплата была»). mongomock + настоящий обработчик:
  ...
  • у охраны (бутылка или наличные) и «нам должны» (не платёж) не спрашиваем;
...
async def main():
    ...
    eq("приход на карту — принято", (st, r["item"].get("pay")), (200, "card"))
    ...
    eq("pay: спрашивать у всех, кроме охраны, «нам должны» и бонуса",
       sorted(k["id"] for k in v["kinds"] if not k["pay"]), ["guard", "owed_us", "upsell"])

    print("── итоги смены: на руках только наличное ──────────────────────")
    # в дне: kfc 35 наличными, парковка 20 наличными, приход 50 на карту,
    # бензин 120 безналом, охрана 40 (наличные), «нам должны» 25
    ...
    eq("расход наличными 35 + 20 + 40 + 25, приход наличными 0", (m["spent"], m["got"]), (120, 0))
    eq("безналом: расход 120, приход 50", (m["spent_card"], m["got_card"]), (120, 50))
    eq("на руках = 500 − 120", m["on_hand"], 380)
    ...
    # расход наличными: 35 + 20 + 40 + 25 (приход на карту — безналом, мимо)
    eq("к сдаче = 500 − расход наличными (без питания: отметки нет)", (j["spend"], j["net"]), (120, 380))
    eq("безналом отдельно: бензин 120 − приход 50", j["spend_card"], 70)
    eq("строки знают, как платили", sorted((x["kind"], x["pay"]) for x in j["items"]),
       [("fuel", "card"), ("guard", ""), ("kfc", "cash"), ("owed_us", ""), ("parking", "cash"), ("we_got", "card")])
