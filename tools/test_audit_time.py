...
async def main():
    ...
    await пересчёт({"p1": 10, "p2": 10, "p3": 5, "p4": 5, "p5": 4, "p6": 6,
                    "p7": 5, "p8": 3, "p9": 2, "p10": 3, "p11": 1, "p12": 5,
                    "p13": 10, "p14": 10, "p31": 2})
    ...
    # ГЛАВНОЕ, ради чего всё затевалось: не успел досканировать — ушёл спать,
    # а ночью недосчитанное продали. Продали — значит оно на полке было.
    await скан("p13", T(22, 17), 8)                       # успел 8 из 10
    await продажа("p13", 2, T(22, 20))                    # ночью продали 2 недосчитанных

    await скан("p14", T(22, 17), 6)                       # успел 6 из 10
    await продажа("p14", 2, T(22, 20))                    # продали 2 — значит были
    ...
    print("── не досчитал, а потом продали — продали, значит были ───────")
    eq("успел 8 из 10, ночью продали 2 — недостачи нет",
       (r["p13"]["expected"], r["p13"]["actual"], r["p13"]["gone"], r["p13"]["diff"]), (10, 8, 2, 0))
    eq("успел 6 из 10, продали 2 — не хватает двух, а не четырёх",
       (r["p14"]["expected"], r["p14"]["actual"], r["p14"]["gone"], r["p14"]["diff"]), (10, 6, 2, 2))
    eq("уехало переездом после подсчёта — тоже зачтено, не пропало", r["p7"]["gone"], 0)
    ...
    eq("в недостаче только настоящее: p4 (3) + p5 (4) + p6 (2) + p14 (2)", t["short_qty"], 11)
    ...
    eq("и её итоги те же", sheet["totals"]["short_qty"], 11)
