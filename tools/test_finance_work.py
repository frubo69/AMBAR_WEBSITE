...
пол зарплаты»; старшие Макар и Стас сменяют друг друга).
  ...
  • дни на работе в месяце: вышел 15 сентября — 16 из 30; уехал 20-го — 20;
    два периода в месяце складываются; без периодов — весь месяц, как было;
...
async def main():
    ...
    eq("уехал 20-го (начало не отмечали) — 20 дней", pay.work_in([{"from": "", "to": "2026-09-20"}], M)["days"], 20)
    eq("и в октябре уже ноль", pay.work_in([{"from": "", "to": "2026-09-20"}], "2026-10")["days"], 0)
    eq("два периода в месяце складываются",
       pay.work_in([{"from": "2026-09-01", "to": "2026-09-10"}, {"from": "2026-09-21", "to": ""}], M)["days"], 20)
    ...
    st, r = await ручка({"name": "Макар", "action": "end", "day": "2026-09-15"})
    eq("Макар уехал 15-го", (st, r.get("ok")), (200, True))
    ...
    eq("Макар уехал — не на работе", (card["work_now"]["on"], card["work_now"]["left"]), (False, "2026-09-15"))
