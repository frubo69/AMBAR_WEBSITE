...
Жизненный цикл записи:
    ...
    deleted   убрана из реестра руками; прежний статус лежит в `was`

В остатке — только active. Этот список однажды отстал от кода: статус deleted
появился позже, а в шапке его не было, и четыре сканера подряд считали убранную
бутылку годной. Поэтому проверять надо «статус равен active», а не перечислять
негодные: перечисление устаревает молча.
...
async def unscanned_by_district(by_pd: dict = None) -> tuple:
    ...
    камера её не видит, ревизия ищет её глазами, а проверка бутылки ответить
    про неё не может.
